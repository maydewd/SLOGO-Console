...
The View will call updateDisplay() on the Controller passing a command String that the user has entered. UpdateDisplay() will return a List of Delta objects to the view, each of which will hold the information for a one step change. To do that the Controller will call parseString on the Parser who will return a List of root Nodes back to it. Each Node will be of a type related to the command that it implements and will have an execute method that returns another Node. Upon receipt of the root Nodes the Controller will call execute() on those in a sequence receiving a List of Deltas as a result. While the Nodes append to the List of Deltas recursively they will call the Controllers command  methods such as moveForward, moveBackward etc that in turn will call the respective methods of the Model who will then adjust the Turtle position. 
