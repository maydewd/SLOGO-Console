...
public class FileSelector extends Selector {
    ...
    public FileSelector (HostServices hostServices,
                         IAdvancedModel model,
                         BaseUIView parent,
                         UIManagerTabInterface uiManager) {
        myFileSelector = new Menu("File");
        myFileSelector.getItems().addAll(makeNew(model, hostServices), makeOpen(model),
                                         makeSave(model), makeExport(model), makeImport(model));
        myManager = uiManager;
        errors = parent;
